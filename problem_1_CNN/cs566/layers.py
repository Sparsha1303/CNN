        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    col_start = j * stride
                    col_end = j * stride + WW
                    input_section = x_padded[n, :, row_start:row_end, col_start:col_end]

                    filtered_section = input_section * w[f]

                    
                    sum_result = np.sum(filtered_section)

              
                    final_result = sum_result + b[f]


                    out[n, f, i, j] = final_result
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    x_section = x[n, c, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
                    out[n, c, i, j] = np.max(x_section)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for f in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    x_section = x[n, f, i*pool_height:i*pool_height+pool_height, j*pool_width:j*pool_width+pool_width]
                    max_x_section = np.max(x_section)
                    x_section = (x_section == max_x_section)
                    dx[n, f, i*pool_height:i*pool_height+pool_height, j*pool_width:j*pool_width+pool_width] = x_section * dout[n, f, i, j]
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dx = dx_hat / np.sqrt(var + eps) + dvar * 2 * (x - mean) / N_prime + dmean / N_prime
